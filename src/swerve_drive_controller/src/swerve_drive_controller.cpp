                                                                ...
                                                                const rclcpp::Duration& delta_time)
{
  ...
  const auto chassis_speeds = Eigen::Vector3d{ last_command_msg->twist.linear.x, last_command_msg->twist.linear.y,
                                               last_command_msg->twist.angular.z };

  // Calculate and set inverse kinematics -> wheel speeds (m/s) -> (rad/s)
  const Eigen::VectorXcd moduleStateReqs = kinematics_ * chassis_speeds * (1 / params_.wheel_radius);
...
CallbackReturn SwerveDriveController::on_configure(const rclcpp_lifecycle::State& previous_state)
{
  ...
  get_node()->create_subscription<TwistStamped>(DEFAULT_COMMAND_TOPIC, rclcpp::SystemDefaultsQoS(),
...
// TODO:
CallbackReturn SwerveDriveController::on_activate(const rclcpp_lifecycle::State& previous_state)
{
  return CallbackReturn::SUCCESS;
}

// TODO:
CallbackReturn SwerveDriveController::on_deactivate(const rclcpp_lifecycle::State& previous_state)
{
  return CallbackReturn::SUCCESS;
}

// TODO:
CallbackReturn SwerveDriveController::on_cleanup(const rclcpp_lifecycle::State& previous_state)
{
  return CallbackReturn::SUCCESS;
}

// TODO:
CallbackReturn SwerveDriveController::on_error(const rclcpp_lifecycle::State& previous_state)
{
  return CallbackReturn::SUCCESS;
}

// TODO:
CallbackReturn SwerveDriveController::on_shutdown(const rclcpp_lifecycle::State& previous_state)
{
  return CallbackReturn::SUCCESS;
}
}  // namespace swerve_drive_controller
