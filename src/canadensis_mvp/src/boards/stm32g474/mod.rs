...
struct STM32G474CyphalClock {
    // TIM2 is a 32-bit timer on the G474
    hw_timer: pac::TIM2,
}
...
fn configure_clk(rcc: &mut pac::RCC) {
    ...
    unsafe {
        // HSE is 24 MHz on the stepper boards and the Nucleo.
        // NOTE IMPORTANT FIXME make sure you update this if the board HSE crystal/xtal changes!
        rcc.pllcfgr().write(|w| {
            w
            .pllsrc().bits(0b11)    // set oscillator input to HSE (24 MHz)
            .plln().bits(32)        // set PLL multiplier to (32 / 1)
            .pllm().bits(3 - 1)      // divide HSE by 3 to get 8 MHz
...
static G_CYPHAL_CLOCK: Mutex<RefCell<Option<STM32G474CyphalClock>>> =
    Mutex::new(RefCell::new(None));

// sorry for the confusing naming, this is the concrete type that implements CyphalClock
pub struct CClock {}
...
impl CClock {
    fn new_singleton(tim2: pac::TIM2, rcc: &mut pac::RCC) -> Self {
        ...
        cortex_m::interrupt::free(|cs| {
            *G_CYPHAL_CLOCK.borrow(cs).borrow_mut() = Some(STM32G474CyphalClock::new(tim2, rcc))
        });
        CClock {}
    }
}
...
pub struct STM32G474CanDriver {
    fdcan: pac::FDCAN1,
}

impl STM32G474CanDriver {
...
impl driver::ReceiveDriver<CClock> for STM32G474CanDriver {
...
impl driver::TransmitDriver<CClock> for STM32G474CanDriver {
...
pub type CanDriver = STM32G474CanDriver;
...
pub fn init() -> (CClock, GClock, CanDriver)
{
    ...
    let hwdriver = STM32G474CanDriver::new_singleton(dp.FDCAN1, dp.GPIOA, &mut rcc);
