...
use canadensis_can::{CanId, CanNodeId, Frame, driver::{ReceiveDriver, TransmitDriver, optimize_filters}};
use fdcan::{
    FdCan, NormalOperationMode,
    config::{ClockDivider, DataBitTiming, FdCanConfig, FrameTransmissionConfig, GlobalFilter, Interrupts, NominalBitTiming, TimestampSource},
    filter::{Action, ExtendedFilter, FilterType},
    frame::{FrameFormat, TxFrameHeader},
    id::{ExtendedId, Id}
};
use stm32g4xx_hal::{can::{Can, CanExt, Rx, Tx}, rcc::Rcc};
use fugit::ExtU32;

pub struct CanDriver<CAN: CanExt> where Can<CAN>: fdcan::Instance {
    hw_can: FdCan<Can<CAN>, NormalOperationMode>,
}

impl<CAN: CanExt> CanDriver<CAN> where Can<CAN>: fdcan::Instance {
    pub fn new<RX: Rx<CAN>, TX: Tx<CAN>>(instance: CAN, rx_pin: RX, tx_pin: TX, rcc: &mut Rcc) -> Self{
...
impl<CL: Clock, CAN: CanExt> ReceiveDriver<CL> for CanDriver<CAN> where Can<CAN>: fdcan::Instance {
...
impl<CL: Clock, CAN: CanExt> TransmitDriver<CL> for CanDriver<CAN> where Can<CAN>: fdcan::Instance {
    type Error = Infallible;
