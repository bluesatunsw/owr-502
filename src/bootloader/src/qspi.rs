...
impl QspiSys {
    pub fn new(
        ...
        res.hw_swpi
            .command(Command::new(DdrMode::Disabled).with_instruction(LineMode::Single, 0xFF));

        // Write Enable for Volatile Status Register
        res.hw_swpi
            .command(Command::new(DdrMode::Disabled).with_instruction(LineMode::Single, 0x50));
        ...
        // Enable QPI
        res.hw_swpi
            .command(Command::new(DdrMode::Disabled).with_instruction(LineMode::Single, 0x38));
...
impl Flash for QspiSys {
    ...
    fn enable_write(&mut self) {
        ...
        // Chip Erase
        self.hw_swpi
            .command(Command::new(DdrMode::Disabled).with_instruction(LineMode::Quad, 0x60));
        self.write_enabled = true;
    }

    fn disable_write(&mut self) {
        self.hw_swpi.memory_mapped(
            IoCommand::new(DdrMode::Disabled, LineMode::Quad)
