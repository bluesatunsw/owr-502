...
impl<T: TransferId + PartialEq> ChunkManager<T> {
    ...
    pub fn complete(&self) -> bool {
        self.chunklets.iter().all(|x|
            if let ChunkletState::Filled = x { true } else { false }
        )
    }
    ...
    pub fn end_transfer(&mut self, id: &T, data: &[u8]) -> bool{
        for i in 0..self.chunklets.len() {
            if let ChunkletState::Transferring(x) = &self.chunklets[i] && x == id {
                let start_index = i*TRANSFER_SIZE;
                (&mut self.data[start_index..(start_index+TRANSFER_SIZE)]).copy_from_slice(&data[0..TRANSFER_SIZE]);
...
impl<T> DoubleBuffer<T> {
    pub fn new(a: T, b: T) -> Self{
        Self { state: false, buf_a: a, buf_b: b }
    }
