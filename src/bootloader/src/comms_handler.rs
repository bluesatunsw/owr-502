...
use crate::{chunk::{ChunkManager, DoubleBuffer, TRANSFER_SIZE}, common::{FlashCommand, LocatedChunk}};
...
struct Update<T: Transport> where T::TransferId: PartialEq {
    pub path: Vec<u8, 255>,
    pub server: T::NodeId,
...
impl<T: Transport> CommsHandler<T> where T::TransferId: PartialEq {
    ...
    pub fn poll(&mut self) -> FlashCommand {
        ...
        && let Some(chunk) = update.buffers.front_mut().extract_data() {
            ...
            FlashCommand::Write(LocatedChunk { data: chunk, location, offset })
        } else {
            FlashCommand::None
        }
    }
...
impl<T: Transport> TransferHandler<T> for CommsHandler<T> where T::TransferId: PartialEq {
    ...
    fn handle_response<N: Node<Transport = T>>(
        ...
        if let Some(update) = &mut self.update {
            ...
            return update.buffers.front_mut().end_transfer(&tid, payload.as_slice())
                || update.buffers.back_mut().end_transfer(&tid, payload.as_slice());
        }
