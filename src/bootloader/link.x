...
{
  /* */
  RESET : ORIGIN = 0x08007F00, LENGTH = 256
  /* Use the first 32k of FLASH only */
  FLASH : ORIGIN = 0x08000000, LENGTH = 32K-256
...
ASSERT(_stext >= ORIGIN(CODE) && _stext < ORIGIN(CODE) + LENGTH(CODE), "
ERROR(cortex-m-rt): The .text section must be placed inside the CODE memory.
Set _stext to an address within the CODE region.");
